...
export function Keypad({ onDigit, onClear, onShuffle }: KeypadProps) {
  return (
    <div
      className="grid grid-cols-3 gap-2.5 w-full max-w-[320px] mx-auto"
      role="group"
...
function Key({
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      className={`min-h-11 h-[clamp(46px,6.5vh,56px)] rounded-2xl text-xl font-semibold tabular-nums border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/70 ${styles}`}
    >
      {label}
    </motion.button>
  );
